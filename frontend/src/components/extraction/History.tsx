...
export const History: React.FC<HistoryProps> = ({ onViewExtraction }) => {
  ...
  const [downloadFormat, setDownloadFormat] = useState<
    Record<number, "json" | "csv" | "excel">
  >({});

  const loadExtractions = useCallback(async () => {
    setLoading(true);
    try {
      const response: ExtractionListResponse = await extractionApi.list(
        page,
        20,
      );
                      ...
                      <div className="flex flex-col gap-1">
                        <select
                          ...
                          onChange={(e) =>
                            setDownloadFormat({
                              ...downloadFormat,
                              [extraction.id]: e.target.value as
                                | "json"
                                | "csv"
                                | "excel",
                            })
                        ...
                        <button
                          onClick={() =>
                            handleDownload(
                              extraction,
                              downloadFormat[extraction.id] || "json",
                            )
                          }
