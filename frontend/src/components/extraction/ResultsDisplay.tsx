...
export const ResultsDisplay: React.FC<ResultsDisplayProps> = ({ extraction }) => {
  const [currentExtraction, setCurrentExtraction] = useState(extraction)
  const [downloading, setDownloading] = useState(false)
  const [downloadFormat, setDownloadFormat] = useState<'json' | 'csv' | 'excel'>('json')
  ...
  useEffect(() => {
    ...
    const shouldStream = currentExtraction.status === 'pending' || currentExtraction.status === 'processing'
    ...
    eventSource.onmessage = (event) => {
      try {
        ...
        if (data.type === 'status_update') {
          ...
          extractionApi.get(currentExtraction.id).then(updated => {
          ...
          console.log(`Step ${data.step}: ${data.message} (${data.time_elapsed}s)`)
        }
      } catch (err) {
  ...
  const timingMetrics = currentExtraction.llm_final_output?.timing_metrics || null
  ...
  return (
    <div className="bg-card p-6 rounded-lg border space-y-4">
      ...
      <div className="space-y-2">
        <div>
          ...
          <span className={`px-2 py-1 rounded text-sm ${
            currentExtraction.status === 'completed' ? 'bg-green-100 text-green-800' :
            currentExtraction.status === 'failed' ? 'bg-red-100 text-red-800' :
        ...
        {currentExtraction.status === 'processing' && currentStep !== null && (
          <div className="space-y-2">
            ...
            <div className="text-xs text-muted-foreground space-y-1">
              <div className={currentStep >= 1 ? 'text-green-600' : currentStep === 0 ? 'text-primary' : ''}>
                {currentStep >= 1 ? '✓' : currentStep === 0 ? '→' : '○'} Step 1: Processing input files
              </div>
              <div className={currentStep >= 2 ? 'text-green-600' : currentStep === 1 ? 'text-primary' : ''}>
                {currentStep >= 2 ? '✓' : currentStep === 1 ? '→' : '○'} Step 2: Extracting tables
              </div>
              <div className={currentStep >= 3 ? 'text-green-600' : currentStep === 2 ? 'text-primary' : ''}>
                {currentStep >= 3 ? '✓' : currentStep === 2 ? '→' : '○'} Step 3: Validating extractions
              </div>
              <div className={currentStep >= 4 ? 'text-green-600' : currentStep === 3 ? 'text-primary' : ''}>
                {currentStep >= 4 ? '✓' : currentStep === 3 ? '→' : '○'} Step 4: Generating final output
              </div>
              <div className={currentStep >= 5 ? 'text-green-600' : currentStep === 4 ? 'text-primary' : ''}>
                {currentStep >= 5 ? '✓' : currentStep === 4 ? '→' : '○'} Step 5: Storing results
              </div>
            </div>
          </div>
        )}
        ...
        {timingMetrics && (
          <div className="text-sm text-gray-600 space-y-1">
            <div>
              ...
              {timingMetrics.total_time != null ? `${timingMetrics.total_time.toFixed(2)}s` : 'N/A'}
            </div>
            <div className="ml-4 space-y-0.5">
              <div>
                Step 1 (File Processing): {timingMetrics.step_1_file_processing != null ? `${timingMetrics.step_1_file_processing.toFixed(2)}s` : 'N/A'}
              </div>
              <div>
                Step 2 (Extraction): {timingMetrics.step_2_extraction != null ? `${timingMetrics.step_2_extraction.toFixed(2)}s` : 'N/A'}
              </div>
              <div>
                Step 3 (Validation): {timingMetrics.step_3_validation != null ? `${timingMetrics.step_3_validation.toFixed(2)}s` : 'N/A'}
              </div>
              <div>
                Step 4 (Finalization): {timingMetrics.step_4_finalization != null ? `${timingMetrics.step_4_finalization.toFixed(2)}s` : 'N/A'}
              </div>
              <div>
                Step 5 (Storage): {timingMetrics.step_5_storage != null ? `${timingMetrics.step_5_storage.toFixed(2)}s` : 'N/A'}
              </div>
            </div>
          </div>
        )}
      ...
      {currentExtraction.status === 'completed' && currentExtraction.table_data && (
          ...
          <div className="border-t pt-4">
            ...
            <div className="flex gap-2 items-center">
              <select
                ...
                onChange={(e) => setDownloadFormat(e.target.value as 'json' | 'csv' | 'excel')}
