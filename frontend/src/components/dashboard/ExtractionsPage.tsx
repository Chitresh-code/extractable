...
export function ExtractionsPage() {
  ...
  const [extractionToEdit, setExtractionToEdit] =
    React.useState<Extraction | null>(null);
  const [deleteDialogOpen, setDeleteDialogOpen] = React.useState(false);
  const [extractionToDelete, setExtractionToDelete] = React.useState<
    number | null
  >(null);
  const [extractionToDeleteName, setExtractionToDeleteName] = React.useState<
    string | null
  >(null);
                        ...
                        <DropdownMenuContent align="end">
                          ...
                          {extraction.status === "completed" && (
                            <>
                              <DropdownMenuItem
                                onClick={() =>
                                  handleDownload(extraction.id, "json")
                                }
                              >
                              ...
                              <DropdownMenuItem
                                onClick={() =>
                                  handleDownload(extraction.id, "csv")
                                }
                              >
                                <Download className="mr-2 h-4 w-4" />
                                Download CSV
                              ...
                              <DropdownMenuItem
                                onClick={() =>
                                  handleDownload(extraction.id, "excel")
                                }
                              >
                                <Download className="mr-2 h-4 w-4" />
                                Download Excel
          ...
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-base pt-2">
              ...
              <span className="font-semibold text-foreground">
                {extractionToDeleteName}
              </span>
              ? This action cannot be undone and will permanently delete the
              extraction and all its data.
            </AlertDialogDescription>
          </AlertDialogHeader>
