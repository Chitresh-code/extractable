...
def build_validation_prompt(extraction_result: Dict[str, Any], image_index: int, total_images: int) -> str:
    """
...
async def validate_extraction(
    ...
    response = await openai_client.generate_content(
        user_id=user_id, prompt=prompt, images=images, complexity=validation_complexity
    )
...
async def validate_extractions(
    user_id: int, extraction_results: List[Dict[str, Any]], image_bytes_list: List[bytes], complexity: str = "regular"
) -> List[Dict[str, Any]]:
    """
    ...
    for (idx, _), result in zip(tasks, validation_results):
        if isinstance(result, Exception):
            # Handle exceptions
            validations.append({"image_index": idx, "error": str(result)})
        else:
