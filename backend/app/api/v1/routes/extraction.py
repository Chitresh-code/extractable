...
async def list_extractions(
    ...
    extractions = db.query(Extraction).filter(
        Extraction.user_id == current_user.id
    ).order_by(Extraction.created_at.desc()).offset(skip).limit(page_size).all()
    
    total = db.query(Extraction).filter(
        Extraction.user_id == current_user.id
    ).count()
...
async def update_extraction(
    ...
    if extraction.status == ExtractionStatus.PROCESSING.value:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Cannot update extraction while it is processing"
        )
