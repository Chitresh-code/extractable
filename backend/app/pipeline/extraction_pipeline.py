...
async def run_extraction_pipeline(db: Session, extraction: Extraction, file=None, files: List = None) -> Extraction:
    """
    ...
    try:
        ...
        await event_manager.broadcast(
            extraction.id,
            {"type": "step_update", "extraction_id": extraction.id, "step": 0, "message": "Starting pipeline..."},
        )
        ...
        if extraction:
            ...
            if not extraction.llm_final_output:
                ...
                try:
                    extraction.llm_final_output = (
                        json.loads(extraction.llm_final_output) if isinstance(extraction.llm_final_output, str) else {}
                    )
                except:
                    extraction.llm_final_output = {}
